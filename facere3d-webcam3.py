...
while True:
    ...
    for loc in locations:
        try:
            enc = face_recognition.face_encodings(rgb_small, [loc])[0]
    ...
    for i, (top, right, bottom, left) in enumerate(valid_locations):
        ...
        if i < len(landmarks_list):
            ...
            # === Disegna mesh dei landmark ===
            for (x, y) in coords_np:
                cv2.circle(frame, (x, y), 1, (0, 255, 255), -1)  # punti gialli

        # === Visualizza bounding box e testi ===
